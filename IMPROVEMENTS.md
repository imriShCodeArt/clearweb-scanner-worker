...
`docker-compose.yml` limits the worker to **4 GB RAM** and **2 CPUs**. `stop_grace_period: 35s` aligns with scan drain timeout.
...
### 11. Request validation with Zod **Done**

`src/lib/validation/scan.ts` validates scan requests — URL length, timeout bounds, optional screenshot flag — with consistent error messages via `formatZodError`.

### 12. ~~Consolidate config~~ ✅

Timeout and navigation settings live in `src/config/env.ts`.

### 13. Clean up dead code **Done**

Removed main-app leftovers that were never wired in the worker:

- `persist.ts` (Drizzle DB writes)
- `stored-result.ts` (DB JSON parsing)
- `log-scan-failure.ts` (Next.js-style console helper)

Scanner lifecycle is explicit: `initScanner(config)` at startup, `getScanner()` for consumers, `closeScanner()` on shutdown.

### 14. Stronger tests **Done**

- Unit tests for `url.ts` (protocol normalization, typos, private hosts)
- Expanded `url-security` edge cases (metadata IP, IPv6-mapped IPv4, dual-stack DNS, empty/failed lookups)
- Optional Playwright integration test: `RUN_INTEGRATION_TESTS=true yarn test:integration`
- Docker CI smoke test queues a real scan of `https://example.com` via `scripts/docker-smoke.sh`
